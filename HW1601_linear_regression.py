...
def compute_cost(X, y, theta):
    ...
    h = np.matrix(X) * np.matrix(theta)
    J = np.sum(np.square(h.T - np.matrix(y))) / (2 * m)
...
def gradient_descent(X, y, theta, alpha, num_iters):
    ...
    for iter in range(num_iters):
        ...
        h = np.matrix(X) * np.matrix(theta)
        theta[0] = theta[0] - alpha * np.sum(h.T - np.matrix(y)) / m
        theta[1] = theta[1] - alpha * ((h.T - np.matrix(y)) * np.matrix(X[:, 1]).T) / m
...
def plot_linear_fit(X, theta):
    ...
    # 使用 legend 命令显示图例，图例显示位置为 "loc='lower right'"
    ...
    h = np.matrix(X) * np.matrix(theta)
    plt.plot(X[:, 1], h, 'b-', label = 'Linear Regression', 
...
def plot_visualize_cost(X, y, theta_best):
    ...
    for i in range(101):
        for j in range(101):
            ...
            theta_val = np.zeros((2, 1))
            theta_val[0] = theta0_vals[i]
            theta_val[1] = theta1_vals[j]
